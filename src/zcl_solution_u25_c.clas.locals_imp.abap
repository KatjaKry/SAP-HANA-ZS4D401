...
CLASS lcl_flight IMPLEMENTATION.
  ...
  METHOD get_description.
    ...
    txt = TEXT-005.
*    txt = 'Flug &carrid1& &connid& am  &date&  von &from&  nach &to&'(005). "Übersetzung -> DE
...
CLASS lcl_passenger_flight DEFINITION INHERITING FROM lcl_flight .
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA flights_buffer TYPE HASHED TABLE OF st_flights_buffer    "Übung 17
    ...
    CLASS-DATA connections_buffer TYPE HASHED  TABLE OF st_connections_buffer      "Übung 16
...
*      "Beispiel wenn der Flug von Frankfurt nach New York 7 Stunden dauert.
*      "Frankfurt UTC+1  New York UTC-5
*      "Lokale Zeit wird umgerechnet auf UTC Zeit -> Frankfurt 11:00 Uhr     UTC -> 10:00 Uhr
...
*
*      "Lokale Zeit wird umgerechnet auf UTC Zeit -> New York  12:00 Uhr     UTC -> 17:00 Uhr
...
*      SORT flights_buffer BY carrier_id connection_id flight_date.  "Übung 14 Übung 16 Auskommentieren
*      DELETE ADJACENT DUPLICATES FROM flights_buffer COMPARING carrier_id connection_id flight_date. "Übung 14
    ENDIF.
...
*    ENDLOOP.
    ...
    "Übung 14 Übung 15 Feldsymol
    r_result = VALUE #( FOR <line> IN flights_buffer                       "Inline Field Symbols <line>
...
*      seats_free = flight_raw-seats_max - flight_raw-seats_occupied.  "Delete Übung 11
      seats_free = flight_raw-seats_free.   "Übung 11
      price     = flight_raw-price.         "Übung 12
* convert currencies                        "Übung 12 Auskommentieren
*      TRY.
  ...
  PUBLIC SECTION.
    ...
    METHODS find_passenger_flight
      IMPORTING
        ...
        i_seats           TYPE i
      EXPORTING
        e_flight          TYPE REF TO lcl_flight  "Übung 20 Up Cast -> Passenger flights
        e_days_later      TYPE i.
    ...
    METHODS find_cargo_flight
      ...
      EXPORTING
        e_flight          TYPE REF TO lcl_flight "Übung 20 Up Cast  -> Cargo flight
        e_days_later      TYPE i.
...
*    DATA cargo_flights TYPE lcl_cargo_flight=>tt_flights.
    ...
    DATA pf_count TYPE i.                "Übung 20. Anzahl der Passagierflüge
...
*    name = carrier_id && ` ` && name.    "Delete Übung 11
    ...
    DATA(passenger_flights) =                                "Übung 20
        lcl_passenger_flight=>get_flights_by_carrier(
              i_carrier_id    = i_carrier_id ).
    pf_count = lines( passenger_flights  ).                  "Übung 20 Anzahl der Flüge


    DATA(cargo_flights) =                                    "Übung 20
        lcl_cargo_flight=>get_flights_by_carrier(
              i_carrier_id    = i_carrier_id ).
    cf_count = lines( cargo_flights  ).                      "Übung 20 Anzahl der Flüge

    LOOP AT passenger_flights INTO DATA(passflight).         "Übung 20 Flüge in die Collection übertragen
      APPEND passflight TO flights.                          "Übung 20 Up cast
    ENDLOOP.

    LOOP AT cargo_flights INTO DATA(cargoflight).            "Übung 20 Flüge in die Collection übertragen
  ...
  METHOD lif_output~get_output.   "Übung 21
    ...
    APPEND |{ 'Passenger Flights:'(002) }  { pf_count } | TO r_result.  "Übung 20
  ...
  METHOD find_cargo_flight.
    ...
    LOOP AT me->flights INTO DATA(flight)
        WHERE table_line->flight_date >= i_from_date AND table_line IS INSTANCE OF lcl_cargo_flight.  "Übung 20
  ...
  METHOD find_passenger_flight.
    ...
    LOOP AT me->flights INTO DATA(flight)
         WHERE table_line->flight_date >= i_from_date AND table_line IS INSTANCE OF lcl_passenger_flight. "Übung 20
...
**Übung 13
*    SELECT FROM /lrn/passflight                               "Variante 1
...
*SELECT FROM /lrn/passflight                                   "Fehler
*      FIELDS SUM( seats_max - seats_occupied ) / COUNT( * )
*          WHERE carrier_id = @carrier_id
*           INTO @DATA(aggregates).

*SELECT FROM /lrn/passflight                                   "Variante 2
*      FIELDS AVG( seats_max - seats_occupied )
*          WHERE carrier_id = @carrier_id
...
*    DATA name TYPE string.
    ...
    AUTHORITY-CHECK OBJECT '/LRN/CARR'
                 ...
                 ID 'ACTVT' FIELD '03'.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_00_failed    "Übung 25
        EXPORTING
          textid     = zcx_00_failed=>carrier_no_read_auth
          carrier_id = i_carrier_id.
    ENDIF.
    "Übung 18 <-
    ...
    SELECT SINGLE
      FROM /dmo/carrier
*       from /LRN/I_Carrier
*    FIELDS name, currency_code       "Delete Übung 11
*     FIELDS concat_with_space( carrier_id, name, 1 ) as name, currency_code    "Übung 11 mit as name
     FIELDS concat_with_space( carrier_id, name, 1 ) AS name, currency_code    "Übung 11 ohne as name
     WHERE carrier_id = @i_carrier_id
     INTO @DATA(details).
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_00_failed   "Übung 25
        EXPORTING
          textid     = zcx_00_failed=>carrier_not_exist
          carrier_id = i_carrier_id.
