...
*    "Test 1
    ...
    "Test 2
    TRY.
        ltcl_flights=>the_carrier = lcl_carrier=>get_instance( i_carrier_id = some_flight_data-carrier_id  ).   "Übung 23
*        DATA(the_carrier) = NEW lcl_carrier( i_carrier_id = some_flight_data-carrier_id  ).  "Falsch

*      ltcl_flights=>the_carrier = the_carrier.

      CATCH cx_abap_auth_check_exception INTO DATA(exc_auth). "Übung 18     Übung 24
         cl_abap_unit_assert=>fail(  exc_auth->get_text(  ) ).  "Übung 18!!!!!!!!!!!!!!!!!!!



      CATCH cx_root INTO DATA(exc_root).   "Übung 18- Das steht "Mutter" except. Klasse, muss zweite stehen oben    ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ
        cl_abap_unit_assert=>fail( 'Die Klasse: lcl_carrier konnet nicht instanziiert werden' ).
    ENDTRY.
